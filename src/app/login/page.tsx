...
export default function LoginPage() {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast({
        ...
        description: "Bem-vindo de volta ao portal da linhagem Amaral.",
      });
      router.push('/painel');
    }, 1500);
  };
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex flex-col items-center justify-center p-6 relative overflow-hidden">
      ...
      <Card className="w-full max-w-md border-2 border-accent/20 shadow-2xl rounded-[3.5rem] bg-white overflow-hidden relative z-10">
        ...
        <CardContent className="space-y-6 p-10 pt-4">
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <Button 
              ...
              className="w-full bg-primary hover:bg-primary/95 py-7 font-bold text-lg uppercase tracking-widest rounded-full shadow-lg transition-transform hover:scale-[1.02]"
            >
              {isLoggingIn ? "Validando Linhagem..." : "Entrar no Portal"}
            </Button>
          </form>
